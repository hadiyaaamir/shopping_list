...
class _Dropdown extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final List<ShoppingListUserRoles> userRolesList = ShoppingListUserRoles
        .values
        .where((role) => role != ShoppingListUserRoles.owner)
        .toList();

    return DropdownButton<ShoppingListUserRoles>(
      value: user.listUser.role,
      items: userRolesList.map((role) {
        return DropdownMenuItem<ShoppingListUserRoles>(
          value: role,
      ...
      onChanged: (newValue) {
        ...
        context.read<ShoppingListUsersBloc>().add(
              ShoppingListUsersEdited(
                editedUser: editedUser,
                onSuccess: () {
                  final List<ShoppingListUser> listUsers = context
                  ...
                  context
                      ...
                      .add(ShoppingListListUsersEdited(listUsers: listUsers));
                },
              ),
            );
      },
...
class _DismissibleListTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final status = context.read<ShoppingListUsersBloc>().state.status;
